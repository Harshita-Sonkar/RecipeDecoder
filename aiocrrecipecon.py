...
def save_recipe_as_html(recipe_data, filepath):
    ...
    html_content = generate_html_from_schema(recipe_data).replace('```', '')
    schema_json = json.dumps(recipe_data).replace('```', '')
    ...
    if not html_content:  # Fallback in case of an error or empty response
        full_html = "<html><body><h1>Error generating recipe content.</h1></body></html>"
